...
//exec/./lle_cpu ~/IMAGES/salinasA.tiff salinasA 3 1
...
int main(int argc, char *argv[])
{
	...
	**/
	...
	for(int ii=0; ii<total_pixels; ii++){
		...
		//VectorXf w = C.colPivHouseholderQr().solve(ones);
	...
	/** minimizing cost aka getting smallest eigenvector/values decomposition***/
	MatrixXf cpu_denseSub = MatrixXf(cpu_M);
	
	SelfAdjointEigenSolver<MatrixXf> es(cpu_denseSub);
	MatrixXf eigenvectors = es.eigenvectors();
	MatrixXf eigenvectors_final = eigenvectors.leftCols(dimsToKeep+1).transpose();
	
	// const int num_eigenvalues = dimsToKeep+1;

	// arpaca::SymmetricEigenSolver<float> solver = arpaca::Solve(cpu_M, num_eigenvalues, arpaca::MAGNITUDE_SMALLEST);//MAGNITUDE_SMALLEST 
	...
	string out = "/home/aramirez39/IMAGES_OUT/" + OUTNAME + "_cpu_lle_F32.tiff";
